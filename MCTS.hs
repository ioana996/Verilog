...
{-
    ...
    * copiii.
-}
data Tree s a = Tree { stateInit :: s
				, actionType :: a
				, nrVisited :: Int
				, score :: Float
				, children :: [Tree s a]
...
-}
data Zipper s a = Zipper { tree :: Tree s a
					, parents :: [Tree s a]
					, generator :: StdGen
					}
...
treeState (Tree stateInit actionType nrVisited score children) = stateInit
...
treeAction (Tree stateInit actionType nrVisited score children) = actionType
...
treeScore (Tree stateInit actionType nrVisited score children) = score
...
treeVisits (Tree stateInit actionType nrVisited score children) = nrVisited
...
treeChildren (Tree stateInit actionType nrVisited score children) = children
...
zipperTree (Zipper tree parents generator) = tree
...
zipperGen (Zipper tree parents generator) = generator
...
expand gen state = makeexpandgen gen state initialaction where initialaction = undefined
makeexpandgen gen state action = (Tree state action 0 0.0 $ foldl (\list pair -> list ++ [makeexpandgen gen (snd pair) (fst pair)]) [] (gen state))
...
rolloutTree (Tree stateInit actionType nrVisited score children) gen = recFunc (Tree stateInit actionType nrVisited score children) gen 
				[(Tree stateInit actionType nrVisited score children)]

recFunc :: GameState s a => Tree s a -> StdGen -> [Tree s a] -> ([Tree s a], Outcome, StdGen)
recFunc (Tree stateInit actionType nrVisited score children) gen drum = 
	if (outcome(stateInit) /= Ongoing) then ((take  (length drum) ([(Tree stateInit actionType nrVisited score children)] ++ drum)), (outcome stateInit), gen) 
	else recFunc (children !! (fst randCoord)) (snd randCoord) ([(Tree stateInit actionType nrVisited score children)] ++ drum)
		  where randCoord = randomR (0, (length children) - 1) gen
...
bestChild (Tree stateInit actionType nrVisited score children) = maximumBy (comparing raport) children

raport :: Tree s a -> Float
raport (Tree state action visited scor copii) = scor/(fromIntegral visited)
...
getZipper tree generator = (Zipper tree [] generator)
...
isRoot (Zipper tree parents generator) = if (null parents) then True
											else False
...
ucb1 :: Float  -- scorul copilului
     ...
     -> Float  -- estimarea
ucb1 childScore childVisits parentVisits = (childScore / (fromIntegral childVisits)) + 2 * sqrt ((log (fromIntegral parentVisits))/(fromIntegral childVisits))
...
select (Zipper tree parents generator) = (Zipper (head (foldl (\ lista elem -> 
											if (null lista) then [elem] ++ lista 
											else if ((ucb1 (treeScore elem) (treeVisits elem)
											(treeVisits (head parents))) > (ucb1 (treeScore (head lista)) (treeVisits (head lista))
											(treeVisits (head parents)))) then [elem] ++ lista 
											else lista )
											[] (treeChildren tree))) ([tree] ++ parents) generator)
...
traverse (Zipper tree parents generator) = until isFinal select (Zipper tree parents generator)
													
isFinal :: Zipper s a -> Bool
isFinal (Zipper tree parents generator) = if (((treeVisits tree) == 0) || ((null (treeChildren tree)) == True)) then True 
												else False
...
toParent (Zipper tree parents generator) = (Zipper (head parents) (tail parents) generator)
